...
public class UserInfoDAO {
	public int createUser(String userName,String userPassword,String email) {
		...
		int count = 0;
		String sql = "insert into user_info(user_name,user_password,email, status, logined, regist_date, update_date)"
				+ " values (?, ?, ?, ?, ?, now(), 0)";
		try {
	...
	public int logout(String email) {
		...
		Connection connection = dbConnector.getConnection();
		int result=0;
		String sql = "update user_info set logined=0 where email=?";
		try {
			PreparedStatement preparedStatement = connection.prepareStatement(sql);
			preparedStatement.setString(1, email);
			result = preparedStatement.executeUpdate();
